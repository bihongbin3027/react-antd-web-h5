...
function Process ({ history, location }) {
  ...
  useEffect(() => {
    ...
    const fetchInfo = async () => {
      ...
      const eventList = await API.eventType({ type: 2, templateId })
      const person = await API.eventPerson({ type: 2, personType: 3, templateId })
      Toast.hide()
      setLoading(true)
      ...
      setTime(detail.handoverEvent.createTime)
      setSupervisor(() => {
      ...
      if (detail.handoverEvent.handleStatus !== '3' && detail.handoverEvent.handleStatus !== '4') {
        // 重复权限弹窗
      ...
      setComplaintObj({
        complaint: detail.handoverEvent.complaint,
        complaintMobile: detail.handoverEvent.complaintMobile,
        complaintReasons: detail.handoverEvent.complaintReasons
      })
    }
    fetchInfo()
  }, [id])
      ...
      <div className='scroll-view footer-reserved event-submie-view'>
        <div className='process-container'>
          <div className='base-div'>
            ...
            <div className='event-info' style={{ paddingTop: 5, paddingBottom: 8 }}>
              <div className='event-info-li info-style'>
                <div className='label-left'>投诉人：</div>
                <div className='cont-right'>{complaintObj.complaint}</div>
              </div>
              <div className='event-info-li info-style'>
                <div className='label-left'>联系电话：</div>
                <div className='cont-right'>{complaintObj.complaintMobile}</div>
              </div>
              <div className='event-info-li info-style'>
                <div className='label-left flex-start'>投诉事由：</div>
                <div className='cont-right'>{complaintObj.complaintReasons}</div>
              </div>
            </div>
                ...
                <div className='base-div event-info-pt'>
                  {
                    handoverHandles.map((item, index) => (
                      <div className='event-info info-style' key={index}>
                        ...
                        <dl className='textarea-dl' style={{ marginTop: '10px' }}>
                          <dt>处理结果：</dt>
                          <dd className='text-cont'>
                            {item.handleResult || '无'}
                          </dd>
